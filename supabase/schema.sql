...
-- ---------------------------------------------------------------------
-- 4e) HESABI SİL
--     Frontend'den:  supabase.rpc('delete_account')
--     Hâlâ eşleşikse hesap SİLİNMİYOR (STILL_MATCHED hatası) — paylaşılan
--     couple verisi (notlar/anılar dahil) bir kişiye değil ikisine ait
--     olduğu için yarısını tutmak mümkün değil; kullanıcı önce
--     "eşleşmeyi kaldır" (leave_couple) demek zorunda.
--
--     Eşleşik değilse auth.users satırını siler (profiles/couple_members/
--     notes/strokes/anilar/bilirmisin_profil zaten "on delete cascade" ile
--     otomatik gider). Ama couple başına tek satır tutan oyun tablolarındaki
--     bazı sütunlar (x_user_id, sira, kazanan, cizen_id, hedef_id,
--     tahmin_eden_id, kirmizi_id, siyah_id, invites.used_by) CASCADE değil —
--     daha önce "eşleşmeyi kaldır" ile ayrıldığı eski couple'lardan (artık
--     couple_members'da izi kalmadığı için hangi couple olduğunu bulamayız)
--     kalan bu satırları burada tek tek tarayıp temizliyoruz, yoksa
--     auth.users silme işlemi FK ihlaliyle patlar.
-- ---------------------------------------------------------------------
